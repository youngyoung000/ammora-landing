...
const wheelSegmentPath = index => {
  const startAngle = index * 36 - 18 + .72
  const endAngle = index * 36 + 18 - .72
  ...
  return (
    <svg className="roulette-wheel-art" viewBox="0 0 500 500" aria-hidden="true">
      <defs>
        ...
        <linearGradient id="premium-segment-violet" x1="210" y1="70" x2="292" y2="270" gradientUnits="userSpaceOnUse"><stop stopColor="#5f55d8"/><stop offset=".46" stopColor="#302b70"/><stop offset="1" stopColor="#16182d"/></linearGradient>
        <linearGradient id="premium-segment-cyan" x1="205" y1="64" x2="296" y2="278" gradientUnits="userSpaceOnUse"><stop stopColor="#349cc2"/><stop offset=".48" stopColor="#205d7a"/><stop offset="1" stopColor="#111b31"/></linearGradient>
        <linearGradient id="premium-segment-mint" x1="206" y1="66" x2="295" y2="278" gradientUnits="userSpaceOnUse"><stop stopColor="#3a9995"/><stop offset=".48" stopColor="#205e66"/><stop offset="1" stopColor="#111a2d"/></linearGradient>
        <linearGradient id="premium-segment-blue" x1="208" y1="62" x2="294" y2="280" gradientUnits="userSpaceOnUse"><stop stopColor="#496fc2"/><stop offset=".5" stopColor="#293f83"/><stop offset="1" stopColor="#15172d"/></linearGradient>
      ...
      {wheelRewards.map((reward, index) => {
        ...
        return <g key={reward}>
          <path className={`premium-segment tier-${tier}${selected ? ' is-selected' : ''}`} d={path} fill={`url(#premium-segment-${palettes[index]})`} filter="url(#premium-segment-shadow)" />
          <path className={`premium-segment-gloss${selected ? ' is-selected' : ''}`} d={path} fill="url(#premium-segment-gloss)" />
          {selected && <path className="premium-segment-trace" d={path} pathLength="1" />}
...
export default function BoostPage() {
  ...
  const spin = () => {
    ...
    window.setTimeout(() => setResultVisible(true), 2770)
    window.setTimeout(() => setSpinPhase('idle'), 3060)
  }
        ...
        <Container>
          ...
          <section className="boost-grid">
            <article id="roulette" className="roulette-panel">
              ...
              <div className="roulette-stage" data-phase={spinPhase} data-result={resultVisible ? 'true' : 'false'}>
                ...
                <div className="roulette-spin-label" aria-hidden="true"><strong>{String(resultVisible ? spinResult : 0).padStart(2, '0')}<small>AP</small></strong></div>
              </div>
